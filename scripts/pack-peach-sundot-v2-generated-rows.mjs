...
const sourceDir = path.join(root, 'public/nft-gen-lab/ai-sources/peach-sundot-v2');
const packDir = path.join(root, 'public/nft-gen-lab/row-packs/peach-sundot-v2');
const atlasPath = path.join(root, 'public/pets/lab-ai-peach-sundot/state-atlas.png');
const cellWidth = 192;
...
function addConnectedHeartGlints(frame, rowIndex, column) {
  const bounds = frameBounds(frame);
  if (!bounds) return;

  const cx = Math.round((bounds.left + bounds.right) / 2);
  const top = bounds.top;
  const glow = [255, 174, 68, 88];
  const bright = [255, 235, 156, 228];
  ...
  drawLine(frame, anchorX, anchorY, anchorX - reach, anchorY - 5, glow, 1);
  drawLine(frame, anchorX, anchorY, anchorX - reach, anchorY - 5, bright, 0);
  drawLine(frame, anchorX, anchorY, anchorX + reach, anchorY - 5 + (column % 2), glow, 1);
  drawLine(frame, anchorX, anchorY, anchorX + reach, anchorY - 5 + (column % 2), bright, 0);
...
async function normalizeComponent(source, info, component, rowIndex, column) {
  ...
  addConnectedHeartGlints(frame, rowIndex, column);
  keyGreen(frame);
  return frame;
}
...
await fs.writeFile(path.join(packDir, 'pack-report.json'), `${JSON.stringify({
  id: 'peach-sundot-v2-generated-pack',
  outputAtlas: path.relative(root, atlasPath),
  note: 'Fresh AI-generated Peach Sundot rows. Packing selects the 8 full character bodies, standardizes body scale, removes chroma green, drops loose generated fragments, and bakes connected warm heart-knot glints into each frame.',
  rows: manifestRows,
}, null, 2)}\n`);
