...
const sourceDir = path.join(root, 'public/nft-gen-lab/ai-sources/ember-kite-v2');
const packDir = path.join(root, 'public/nft-gen-lab/row-packs/ember-kite-v2');
const atlasPath = path.join(root, 'public/pets/lab-ai-ember-kite/state-atlas.png');
const cellWidth = 192;
...
function addConnectedKiteGlints(frame, rowIndex, column) {
  const bounds = frameBounds(frame);
  if (!bounds) return;

  const cx = Math.round((bounds.left + bounds.right) / 2);
  const top = bounds.top;
  const glow = [255, 91, 30, 94];
  const bright = [255, 205, 121, 230];
  const quiet = rowIndex === 8 || rowIndex === 9 || rowIndex === 11;
  const intense = rowIndex === 4 || rowIndex === 6 || rowIndex === 10;
  const sway = Math.round(Math.sin(column * 0.9 + rowIndex * 0.4) * 3);
  const anchorX = cx + sway;
  const anchorY = top + 28 + Math.round(Math.cos(column * 0.7) * 2);
  const reach = quiet ? 4 : intense ? 13 : 8;

  drawLine(frame, anchorX, anchorY, anchorX - reach, anchorY - 8, glow, 1);
  drawLine(frame, anchorX, anchorY, anchorX - reach, anchorY - 8, bright, 0);
  drawLine(frame, anchorX, anchorY, anchorX + reach, anchorY - 8 + (column % 2), glow, 1);
  drawLine(frame, anchorX, anchorY, anchorX + reach, anchorY - 8 + (column % 2), bright, 0);
  drawLine(frame, anchorX, anchorY, anchorX, anchorY + Math.max(3, Math.round(reach * 0.45)), glow, 1);
  drawLine(frame, anchorX, anchorY, anchorX, anchorY + Math.max(3, Math.round(reach * 0.45)), bright, 0);

  if (intense || column % 4 === 2) {
    drawDot(frame, anchorX + Math.round(reach * 0.55), anchorY - 6, bright, 1);
  }
}

function targetFrameMetrics(rowIndex) {
  if (rowIndex === 11) {
    return { targetHeight: 140, maxWidth: 184, maxHeight: 184 };
  }

  return { targetHeight: 176, maxWidth: 166, maxHeight: 196 };
}
...
async function normalizeComponent(source, info, component, rowIndex, column) {
  ...
  const { targetHeight, maxWidth, maxHeight } = targetFrameMetrics(rowIndex);
  const scale = Math.min(targetHeight / height, maxWidth / width, maxHeight / height, 1.22);
  ...
  addConnectedKiteGlints(frame, rowIndex, column);
  keyGreen(frame);
  return frame;
}
...
await fs.writeFile(path.join(packDir, 'pack-report.json'), `${JSON.stringify({
  id: 'ember-kite-v2-generated-pack',
  outputAtlas: path.relative(root, atlasPath),
  note: 'Fresh AI-generated Ember Kite rows. Packing selects the 8 full character bodies, standardizes body scale, removes chroma green, drops loose generated fragments, and bakes connected copper kite-loop glints into each frame.',
  rows: manifestRows,
}, null, 2)}\n`);
