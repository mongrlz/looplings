...
const sourceDir = path.join(root, 'public/nft-gen-lab/ai-sources/violet-bow-v2');
const packDir = path.join(root, 'public/nft-gen-lab/row-packs/violet-bow-v2');
const atlasPath = path.join(root, 'public/pets/lab-ai-violet-bow/state-atlas.png');
const cellWidth = 192;
...
function addConnectedBowGlints(frame, rowIndex, column) {
  const bounds = frameBounds(frame);
  if (!bounds) return;

  const cx = Math.round((bounds.left + bounds.right) / 2);
  const top = bounds.top;
  const glow = [145, 93, 255, 92];
  const bright = [224, 207, 255, 224];
  ...
  const anchorX = cx - 6 + sway;
  const anchorY = top + 25 + Math.round(Math.cos(column * 0.7) * 2);
  ...
  drawLine(frame, anchorX, anchorY, anchorX + reach, anchorY - 3 + (column % 2), glow, 1);
  drawLine(frame, anchorX, anchorY, anchorX + reach, anchorY - 3 + (column % 2), bright, 0);
...
async function normalizeComponent(source, info, component, rowIndex, column) {
  ...
  addConnectedBowGlints(frame, rowIndex, column);
  keyGreen(frame);
  return frame;
}
...
await fs.writeFile(path.join(packDir, 'pack-report.json'), `${JSON.stringify({
  id: 'violet-bow-v2-generated-pack',
  outputAtlas: path.relative(root, atlasPath),
  note: 'Fresh AI-generated Violet Bow rows. Packing selects the 8 full character bodies, standardizes body scale, removes chroma green, drops loose generated fragments, and bakes connected violet bow-loop glints into each frame.',
  rows: manifestRows,
}, null, 2)}\n`);
