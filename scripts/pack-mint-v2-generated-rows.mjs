...
const sourceDir = path.join(root, 'public/nft-gen-lab/ai-sources/mint-v2');
const packDir = path.join(root, 'public/nft-gen-lab/row-packs/mint-v2');
const atlasPath = path.join(root, 'public/pets/mint/state-atlas.png');
const cellWidth = 192;
...
function keyGreen(data) {
  for (let index = 0; index < data.length; index += 4) {
    ...
    const mintDominance = g - Math.max(r, b);
    const isChroma = g > 210 && r < 70 && b < 90 && mintDominance > 145;
    const isBrightFringe = g > 185 && r < 95 && b < 110 && mintDominance > 90;
    ...
    if (g > 145 && mintDominance > 75 && r < 125 && b < 135) {
...
function addConnectedHaloGlints(frame, rowIndex, column) {
  ...
  const haloGlow = [90, 238, 226, 94];
  const haloBright = [206, 255, 249, 225];
  ...
  drawLine(frame, anchorX, anchorY, anchorX - reach, anchorY - 3 + (column % 2), haloGlow, 1);
  drawLine(frame, anchorX, anchorY, anchorX - reach, anchorY - 3 + (column % 2), haloBright, 0);

  if (intense || column % 3 === 0) {
    drawLine(frame, anchorX + 8, anchorY - 6, anchorX + 15, anchorY - 9 + (column % 2), haloGlow, 1);
    drawLine(frame, anchorX + 8, anchorY - 6, anchorX + 15, anchorY - 9 + (column % 2), haloBright, 0);
  }
}
...
await fs.writeFile(path.join(packDir, 'pack-report.json'), `${JSON.stringify({
  id: 'mint-v2-generated-pack',
  outputAtlas: path.relative(root, atlasPath),
  note: 'Fresh AI-generated Mint rows. Packing selects the 8 full character bodies, removes chroma green, drops loose generated halo/gleam fragments, and bakes connected mint halo gleams into each frame.',
  rows: manifestRows,
}, null, 2)}\n`);
