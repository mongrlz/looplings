...
const sourceDir = path.join(root, 'public/nft-gen-lab/ai-sources/verdant-leaf-v2');
const packDir = path.join(root, 'public/nft-gen-lab/row-packs/verdant-leaf-v2');
const atlasPath = path.join(root, 'public/pets/lab-ai-verdant-leaf/state-atlas.png');
const cellWidth = 192;
...
function keyGreen(data) {
  for (let index = 0; index < data.length; index += 4) {
    ...
    const isBrightFringe = g > 185 && r < 95 && b < 110 && greenDominance > 90;

    if (isChroma || isBrightFringe) {
      data[index] = 0;
      data[index + 1] = 0;
      data[index + 2] = 0;
      data[index + 3] = 0;
    ...
    if (g > 145 && greenDominance > 75 && r < 125 && b < 135) {
      data[index + 1] = Math.max(data[index], data[index + 2], 92);
    }
  }
}
...
function addConnectedLeafGlints(frame, rowIndex, column) {
  const bounds = frameBounds(frame);
  if (!bounds) return;

  const cx = Math.round((bounds.left + bounds.right) / 2);
  const anchorY = bounds.top + 22 + Math.round(Math.cos(column * 0.7) * 2);
  const anchorX = cx + Math.round(Math.sin(column * 0.65 + rowIndex) * 4);
  const leafGlow = [162, 218, 94, 104];
  const leafBright = [216, 248, 130, 230];
  const quiet = rowIndex === 8 || rowIndex === 9 || rowIndex === 11;
  const intense = rowIndex === 4 || rowIndex === 10 || rowIndex === 7;
  const reach = quiet ? 5 : intense ? 13 : 9;

  drawLine(frame, anchorX, anchorY, anchorX - reach, anchorY - 3 + (column % 2), leafGlow, 1);
  drawLine(frame, anchorX, anchorY, anchorX - reach, anchorY - 3 + (column % 2), leafBright, 0);

  if (intense || column % 3 === 0) {
    drawLine(frame, anchorX + 8, anchorY - 6, anchorX + 15, anchorY - 9 + (column % 2), leafGlow, 1);
    drawLine(frame, anchorX + 8, anchorY - 6, anchorX + 15, anchorY - 9 + (column % 2), leafBright, 0);
  }
}
...
async function normalizeComponent(source, info, component, rowIndex, column) {
  ...
  addConnectedLeafGlints(frame, rowIndex, column);
  keyGreen(frame);
  return frame;
}
...
await fs.writeFile(path.join(packDir, 'pack-report.json'), `${JSON.stringify({
  id: 'verdant-leaf-v2-generated-pack',
  outputAtlas: path.relative(root, atlasPath),
  note: 'Fresh AI-generated Verdant Leaf rows. Packing selects the 8 full character bodies, removes chroma green, drops loose generated leaf/glint fragments, and bakes connected leaf antenna glints into each frame.',
  rows: manifestRows,
}, null, 2)}\n`);
