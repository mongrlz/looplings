...
const sourceDir = path.join(root, 'public/nft-gen-lab/ai-sources/graphite-v2');
const packDir = path.join(root, 'public/nft-gen-lab/row-packs/graphite-v2');
const atlasPath = path.join(root, 'public/pets/graphite/state-atlas.png');
const cellWidth = 192;
...
function addConnectedHaloGlints(frame, rowIndex, column) {
  ...
  const haloGlow = [70, 160, 255, 96];
  const haloBright = [190, 230, 255, 226];
...
await fs.writeFile(path.join(packDir, 'pack-report.json'), `${JSON.stringify({
  id: 'graphite-v2-generated-pack',
  outputAtlas: path.relative(root, atlasPath),
  note: 'Fresh AI-generated Graphite rows. Packing selects the 8 full character bodies, removes chroma green, drops loose generated halo/gleam fragments, and bakes connected cool-blue halo glints into each frame.',
  rows: manifestRows,
}, null, 2)}\n`);
