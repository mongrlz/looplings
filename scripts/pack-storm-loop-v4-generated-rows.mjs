...
const sourceDir = path.join(root, 'public/nft-gen-lab/ai-sources/storm-loop-v4');
const packDir = path.join(root, 'public/nft-gen-lab/row-packs/storm-loop-v4');
const atlasPath = path.join(root, 'public/pets/lab-ai-storm-loop/state-atlas.png');
const cellWidth = 192;
...
function keyGreen(data) {
  for (let index = 0; index < data.length; index += 4) {
    ...
    if (g > 120 && r < 135 && b < 165 && greenDominance > 18) {
      data[index + 3] = 0;
      data[index] = 0;
      data[index + 1] = 0;
      data[index + 2] = 0;
    ...
    if (g > 86 && greenDominance > 34 && r < 120 && b < 145) {
      data[index + 3] = 0;
      data[index] = 0;
      data[index + 1] = 0;
      data[index + 2] = 0;
...
function addConnectedSparks(frame, rowIndex, column) {
  const bounds = frameBounds(frame);
  if (!bounds) return;

  const cx = Math.round((bounds.left + bounds.right) / 2);
  const top = bounds.top;
  const anchorX = cx + Math.round(Math.sin(column * 0.9 + rowIndex) * 6);
  const anchorY = top + 22 + Math.round(Math.cos(column * 0.7) * 3);
  const bright = [228, 252, 255, 255];
  const blue = [32, 206, 255, 245];
  const glow = [22, 122, 255, 92];
  const quiet = rowIndex === 9 || rowIndex === 11;
  const intense = rowIndex === 4 || rowIndex === 10;
  const reach = quiet ? 8 : intense ? 22 : 14;

  drawLine(frame, anchorX, anchorY, anchorX - reach, anchorY - 5 + (column % 3), glow, 1);
  drawLine(frame, anchorX, anchorY, anchorX - reach, anchorY - 5 + (column % 3), quiet ? [76, 122, 170, 200] : blue, 0);

  if (!quiet || column % 3 === 0) {
    drawLine(frame, anchorX - reach, anchorY - 5 + (column % 3), anchorX - reach - 7, anchorY + 1, bright, 0);
  }

  if (intense || rowIndex === 6 || rowIndex === 7) {
    drawLine(frame, anchorX + 12, anchorY - 9, anchorX + 24, anchorY - 13 + (column % 2), glow, 1);
    drawLine(frame, anchorX + 12, anchorY - 9, anchorX + 24, anchorY - 13 + (column % 2), blue, 0);
    drawLine(frame, anchorX + 24, anchorY - 13 + (column % 2), anchorX + 30, anchorY - 8, bright, 0);
  }
}
...
async function normalizeComponent(source, info, component, rowIndex, column) {
  ...
  const maxWidth = 168;
  const maxHeight = 198;
  const scale = Math.min(maxWidth / width, maxHeight / height, 1.2);
  ...
  addConnectedSparks(frame, rowIndex, column);
  keyGreen(frame);
  return frame;
}
...
async function packRow(rowIndex, rowId) {
  ...
  const components = connectedComponents(data, info.width, info.height)
    .filter((component) => component.area >= 2800 && component.width >= 45 && component.height >= 70)
...
await fs.writeFile(path.join(packDir, 'pack-report.json'), `${JSON.stringify({
  id: 'storm-loop-v4-generated-pack',
  outputAtlas: path.relative(root, atlasPath),
  note: 'Fresh AI-generated Storm Loop rows. Packing selects the 8 full character bodies, removes chroma green, drops loose generated spark blobs, and bakes connected antenna sparks into each frame.',
  rows: manifestRows,
}, null, 2)}\n`);
