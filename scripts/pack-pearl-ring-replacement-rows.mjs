...
const sourceDir = path.join(root, 'public/nft-gen-lab/ai-sources/pearl-ring-v2');
const packDir = path.join(root, 'public/nft-gen-lab/row-packs/pearl-ring-v2');
const atlasPath = path.join(root, 'public/pets/lab-ai-pearl-ring/state-atlas.png');
const cellWidth = 192;
...
function keyGreen(data) {
  for (let index = 0; index < data.length; index += 4) {
    ...
    if (g > 125 && r < 135 && b < 160 && greenDominance > 22) {
      data[index] = 0;
      data[index + 1] = 0;
      data[index + 2] = 0;
      data[index + 3] = 0;
    ...
    if (g > r + 20 && g > b + 20 && r < 155 && b < 170) {
      data[index + 1] = Math.max(data[index], data[index + 2]);
    }
  }
}
...
function addConnectedPearlGleam(frame, rowIndex, column) {
  const bounds = frameBounds(frame);
  if (!bounds) return;
  const cx = Math.round((bounds.left + bounds.right) / 2);
  const anchorX = cx + Math.round(Math.sin(column * 0.7 + rowIndex) * 3);
  const anchorY = bounds.top + 18 + Math.round(Math.cos(column * 0.6) * 2);
  const glow = [190, 232, 255, 86];
  const bright = [240, 252, 255, 210];
  const quiet = rowIndex === 8;
  const reach = quiet ? 4 : rowIndex === 4 ? 10 : 7;

  drawLine(frame, anchorX, anchorY, anchorX + reach, anchorY - 3, glow, 1);
  drawLine(frame, anchorX, anchorY, anchorX + reach, anchorY - 3, bright, 0);
}
...
async function normalizeComponent(source, info, component, rowIndex, column) {
  ...
  const maxWidth = 166;
  const maxHeight = 196;
  const scale = Math.min(maxWidth / width, maxHeight / height, 1.16);
  ...
  addConnectedPearlGleam(frame, rowIndex, column);
  keyGreen(frame);
  return frame;
}
...
async function packRow({ row, id }) {
  const sourcePath = path.join(sourceDir, `${String(row).padStart(2, '0')}-${id}-source.png`);
  ...
  if (components.length !== columns) {
    throw new Error(`${id} expected 8 character components, found ${components.length}`);
  }

  const frames = [];
  for (let column = 0; column < columns; column += 1) {
    frames.push(await normalizeComponent(data, info, components[column], row, column));
  }

  const rowBuffer = composeRow(frames);
  const outPath = path.join(packDir, `${String(row).padStart(2, '0')}-${id}-row.png`);
...
async function writeRowIntoAtlas(atlas, atlasWidth, rowIndex, rowBuffer) {
  for (let y = 0; y < cellHeight; y += 1) {
    for (let x = 0; x < cellWidth * columns; x += 1) {
      const sourceIndex = idx(x, y, cellWidth * columns);
      const targetIndex = idx(x, rowIndex * cellHeight + y, atlasWidth);
      atlas[targetIndex] = rowBuffer[sourceIndex];
      atlas[targetIndex + 1] = rowBuffer[sourceIndex + 1];
      atlas[targetIndex + 2] = rowBuffer[sourceIndex + 2];
      atlas[targetIndex + 3] = rowBuffer[sourceIndex + 3];
    }
  }
}
...
for (const replacement of replacementRows) {
  manifestRows.push(await packRow(replacement));
  const rowPath = path.join(packDir, `${String(replacement.row).padStart(2, '0')}-${replacement.id}-row.png`);
  packedRows.set(replacement.row, await sharp(rowPath).ensureAlpha().raw().toBuffer());
}

const { data: atlas, info } = await sharp(atlasPath).ensureAlpha().raw().toBuffer({ resolveWithObject: true });
if (info.width !== cellWidth * columns || info.height !== cellHeight * atlasRows) {
  throw new Error(`Unexpected atlas size ${info.width}x${info.height}`);
}

for (const [rowIndex, rowBuffer] of packedRows.entries()) {
  await writeRowIntoAtlas(atlas, info.width, rowIndex, rowBuffer);
}
...
await fs.writeFile(path.join(packDir, 'pack-report.json'), `${JSON.stringify({
  id: 'pearl-ring-v2-replacement-pack',
  outputAtlas: path.relative(root, atlasPath),
  note: 'Fresh AI-generated replacement rows for Pearl Ring: trade_win, trade_loss, posting, receiving, and sleeping. Packing selects 8 full character bodies, removes chroma green, and bakes connected pearl-ring gleams into each frame.',
  rows: manifestRows,
}, null, 2)}\n`);

console.log(JSON.stringify({
  atlas: path.relative(root, atlasPath),
  replacedRows: manifestRows.map((row) => row.id),
}, null, 2));
